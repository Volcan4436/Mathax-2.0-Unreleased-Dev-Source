...
public class FontFaceSetting extends Setting<FontFace> {
    public FontFaceSetting(String name, String description, FontFace defaultValue, Consumer<FontFace> onChanged, Consumer<Setting<FontFace>> onModuleEnabled, IVisible visible) {
        super(name, description, defaultValue, onChanged, onModuleEnabled, visible);
    }

    @Override
    protected FontFace parseImpl(String string) {
    ...
    protected JSONObject save(JSONObject json) {
        ...
        json.put("type", get().info().type().name());
    ...
    protected FontFace load(JSONObject json) {
        String family = json.has("family") ? json.getString("family") : defaultValue.info().family();

        FontFace.Type type = FontFace.Type.Regular;
        if (json.has("type")) {
            try {
                type = FontFace.Type.valueOf(json.getString("type"));
            } catch (IllegalArgumentException ignored) {
